...
class Option extends \Magento\Bundle\Block\Catalog\Product\Price
{
    ...
    /**
     * Get Required field untuk option/aksesoris bundle option
     * added by ahmad 15/03/2018
     *
     * @return mixed
     *
     */
    ...
    /**
     * Chcek get data qty
     * added by ahmad 15/3/2018
     * @param $option
     * @return mixed
     */
    public function getQtyDataChck($option){
        $qtydata = $this->getProduct()->getPreconfiguredValues()->getData('bundle_option_qty/', $option->getId());
        //$test = $this->getRootDirectory()->getAbsolutePath($this->get);
        return $qtydata;
    }
    ...
    /**
     * Check selection untuk chek apakah option bundle diselect atau tidak
     * Added by ahmd 03/09/2018
     *
     * @param $selection
     * @return bool
     */
    public function isSelectGetQty($selection){
        $selectedOptions = $this->_getSelectedOptions();
        if (is_numeric($selectedOptions)){
            $selection->getSelectionId() == $selectedOptions;
        }else if (is_array($selectedOptions) && !empty($selectedOptions)){
            in_array($selection->getSelectionId(), $selectedOptions);
        }else if ($selectedOptions == 'None'){
            return false;
        }else{
            $selection->getIsDefault() && $selection->isSaleable();
        }
    }
    ...
    /**
     * get array image gallery
     * return array image dari collection
     * * Added by Ahmad 03/08/2018
     *
     * @return \Magento\Framework\DataObject[]]
     */
    public function getImageGallery(){
        $gallery = $this->getProduct()->getMediaGalleryImages();
        // all item data object (images)
        return $gallery->getItems();
    }


    /**
     * Get Qty product option/ qty di bundle bukan qty catalog (call from xml checkbox.phtml)
     *  Added by @Ahmd 03/05/2018
     *
     * @param $selection
     * @return mixed
     */
    public function getQtyOption($selection){
        $this->setFormatProduct($selection);
        $qtyProduct = $selection->getSelectionQty();
