 ...
 */
namespace Magento\ImportProducts\Model\Export\Entity;

use Magento\Framework\App\ResourceConnection;
use Magento\ImportProducts\Model\Export\Adapter\AbstractAdapter;
...
abstract class AbstractEntity
{
    ...
    protected function _getExportAttrCodes()
    {
        if (null === self::$attrCodes) {
            if (!empty($this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_SKIP]) && is_array(
                $this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_SKIP]
            )
            ) {
                $skipAttr = array_flip($this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_SKIP]);
            } else {
                $skipAttr = [];
            }
            $attrCodes = [];
    ...
    {
        if (!isset(
            $this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_GROUP]
        ) || !is_array(
            $this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_GROUP]
        )
        ) {
            ...
            $exportFilter = $this->_parameters[\Magento\ImportProducts\Model\Export::FILTER_ELEMENT_GROUP];
        }
        ...
        foreach ($this->filterAttributeCollection($this->getAttributeCollection()) as $attribute) {
            ...
            if (isset($exportFilter[$attrCode])) {
                $attrFilterType = \Magento\ImportProducts\Model\Export::getAttributeFilterType($attribute);

                if (\Magento\ImportProducts\Model\Export::FILTER_TYPE_SELECT == $attrFilterType) {
                ...
                } elseif (\Magento\ImportProducts\Model\Export::FILTER_TYPE_INPUT == $attrFilterType) {
                    if (is_scalar($exportFilter[$attrCode]) && trim($exportFilter[$attrCode])) {
                        $collection->addAttributeToFilter($attrCode, ['like' => "%{$exportFilter[$attrCode]}%"]);
                    }
                } elseif (\Magento\ImportProducts\Model\Export::FILTER_TYPE_DATE == $attrFilterType) {
                ...
                } elseif (\Magento\ImportProducts\Model\Export::FILTER_TYPE_NUMBER == $attrFilterType) {
                    if (is_array($exportFilter[$attrCode]) && count($exportFilter[$attrCode]) == 2) {
                        $from = array_shift($exportFilter[$attrCode]);
                        $to = array_shift($exportFilter[$attrCode]);
     ...
     * @return \Magento\ImportProducts\Model\Import\Entity\AbstractEntity
     */
