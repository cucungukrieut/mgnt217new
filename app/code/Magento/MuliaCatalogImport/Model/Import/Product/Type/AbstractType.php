 ...
 */
namespace Magento\MuliaCatalogImport\Model\Import\Product\Type;

use Magento\Framework\App\ResourceConnection;
use Magento\MuliaCatalogImport\Model\Import\Product\RowValidatorInterface;
use Magento\MuliaCatalogImport\Model\Import\Product;
...
abstract class AbstractType
{
    ...
    public function __construct(
        ...
        if ($this->isSuitable()) {
            if (!isset($params[0])
                || !isset($params[1])
                || !is_object($params[0])
                || !$params[0] instanceof \Magento\MuliaCatalogImport\Model\Import\Product
            ) {
     ...
     * @return \Magento\MuliaCatalogImport\Model\Import\Product\Type\AbstractType
    ...
    protected function _getProductAttributes($attrSetData)
    {
        if (is_array($attrSetData)) {
            return $this->_attributes[$attrSetData[\Magento\MuliaCatalogImport\Model\Import\Product::COL_ATTR_SET]];
        } else {
    ...
    public function isRowValid(array $rowData, $rowNum, $isNewProduct = true)
    {
        ...
        if (\Magento\MuliaCatalogImport\Model\Import\Product::SCOPE_NULL != $rowScope
            && !empty($rowData[\Magento\MuliaCatalogImport\Model\Import\Product::COL_SKU])
        ) {
            foreach ($this->_getProductAttributes($rowData) as $attrCode => $attrParams) {
                ...
                if (isset($rowData[$attrCode]) && strlen($rowData[$attrCode])) {
                    ...
                    if (\Magento\MuliaCatalogImport\Model\Import\Product::SCOPE_DEFAULT == $rowScope &&
                        ($isNewProduct ||
                        array_key_exists(
                            $attrCode,
                            $rowData
